...
@Observable class AddDeviceViewModel {
    ...
    /// returns the bridge scanner when `VATTNA_BLE_BRIDGE` is set.
    ...
    /// default; in DEBUG, the bridge scanner when `VATTNA_BLE_BRIDGE` is set.
    static func defaultDiscovery() -> DeviceDiscovery {
        ...
        AppLogger.ble.info("🔍 Add Device using CoreBluetooth scan (VATTNA_BLE_BRIDGE not set — iOS Simulator has no Bluetooth)")
        #endif
