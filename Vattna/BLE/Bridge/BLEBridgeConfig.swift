...
//  Vattna
//
//  Debug-only: reads VATTNA_BLE_BRIDGE=host:port once at launch. When set,
...
enum BLEBridgeConfig {
    ...
    static let endpoint: (host: String, port: UInt16)? = {
        guard let raw = ProcessInfo.processInfo.environment["VATTNA_BLE_BRIDGE"] else { return nil }
